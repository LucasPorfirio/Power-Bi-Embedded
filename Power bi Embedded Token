...
public static async Task<HttpResponseMessage> Run(HttpRequest req, ILogger log)
{
    ...
    string reportteste = req.Query["reportid"];
    string groupteste = req.Query ["groupid"];
    // Get access token 
    ...
    var embedUrl =
        await powerBiClient.GetAsync($"https://api.powerbi.com/v1.0/myorg/groups/{groupteste}/reports/{reportteste}")
    ...
    var embedToken = await powerBiClient.PostAsync($"https://api.powerbi.com/v1.0/myorg/groups/{groupteste}/reports/{reportteste}/GenerateToken", tokenContent)
        .ContinueWith<string>((response) =>
        {
            log.LogInformation(response.Result.StatusCode.ToString());
            log.LogInformation(response.Result.ReasonPhrase.ToString());
